...
## Rendez-vous (Semáforos no Papel)
...
* **named semaphores** recebem um **nome globalmente único no sistema operacional** e são compartilhados entre **processos diferentes** e independentes, inclusive que não têm relação de parentesco (como pai-filho).
* **unnamed semaphores** não têm nome, são criados como **variáveis** local ou global e são acessados diretamente pelas **threads** de um mesmo processo.
...
!!! tip
    Esta aula contém atividade para entrega! Trabalhe na pasta `atv/13-barrier` que está no seu repositório de entregas! Leia o `README.md`!
...
!!! done
    Acabou? Garanta que seu resultado está na pasta `13-barrier` do repositório de atividades e faça a submissão da tag!
