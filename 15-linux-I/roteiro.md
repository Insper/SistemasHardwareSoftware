...
Vamos primeiro fazer o download do kernel do Linux no [site oficial](https://www.kernel.org/)([https://www.kernel.org/](https://www.kernel.org/)). Para este roteiro escolheremos a versão `5.0.9`.
...
<div class="alert"> Não se esqueça de usar o comando `w` para salvar as partições criadas para `raiz.img` </div>
...
>                     -kernel linux-5.0.9/arch/x86_64/boot/bzImage \
...
* `-kernel linux-5.0.9/arch/x86_64/boot/bzImage`: instrui o *QEmu* a carregar o kernel presente no caminho passado.
* `-append "quiet init=/bin/sh root=PARTUUID=ac18c11f-01"`: estas opções são passadas para o kernel e configuram sua execução
...
* Crie um usuário novo com seu nome usando o comando `adduser`. Mostre que você consegue logar com seu novo usuário e salvar dados em sua pasta home (crie um gif)
...
Seu arquivo `raiz.img` (zipado) deverá ser entregue juntamente com as respostas deste roteiro. 
